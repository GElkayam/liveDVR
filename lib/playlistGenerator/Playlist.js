...
* */
function Playlist(loggerInfo, serializationCtx, playlistType) {
...
var calcClipStartTimeAndDuration = function(index){
    ...
    if(_.size(ranges) > 0) {
        if(_.every(ranges,rangeOverlap,ranges[0])){
            retVal.min = _.min(ranges,function(r){return r.min;}).min;
...
Playlist.prototype.serializeFrom = function (playlistJSON,loggerInfo,cbDone,reportError){
    var playlist = undefined;
    try {
        playlist = JSON.parse(playlistJSON);
        playlist = new Playlist(loggerInfo, playlist, undefined);
    } catch (err) {
        loggerModule.getLogger("Playlist.serializeFrom", loggerInfo + " ").warn('Unable to un-serialize playlist. Data loss is inevitable!');
        if(reportError) {
           playlist = err;
        } else {
           playlist = new Playlist(loggerInfo, undefined, undefined);
        }
    }
    if(cbDone){
        cbDone(playlist);
...
Playlist.prototype.toJSON = function() {
    ...
    let totalAllowedClips = Number(maxClipsPerFlavor) + 1;
    let nonEmptyClips = _.reduce(obj.sequences[0].clips, (res, clip) => {
        if (!clip.isEmpty() > 0) {
            return res + 1;
        } else {
            return res;
    ...
    if (nonEmptyClips > totalAllowedClips) {
        that.logger.error(`A bug found!!! total clips > maxClipsPerFlavor [${nonEmptyClips} > ${totalAllowedClips}]`);
    } else {
        that.logger.debug("toJSON: total sequences: %d, total clips: %d total non-empty clips: %d, max allowed clips: %d", obj.sequences.length, obj.clipTimes.length, nonEmptyClips, totalAllowedClips);
    }
